...
# plt.show()
#### train
index_class = [np.where(syn_data['trainY'] == 0)[0], np.where(syn_data['trainY'] == 1)[0],
               np.where(syn_data['trainY'] == 2)[0]]
data_class = [syn_data['trainX'][index_class[0], :], syn_data['trainX'][index_class[1], :],
              syn_data['trainX'][index_class[2], :]]

# def classify_two_class(data_0,label_0,data_1,label_1):
epsilon = 3
index0 = 0
index1 = 1
index2 = 2
    ...
    for i in range(0, len(data_x)):
        ...
        for j in range(0, solution[1].shape[0]):
            ...
            for k in range(0, solution[1].shape[1]):
                if solution[1][j, k] != 0:
                    temp = temp & (
                                (data_x[i, 1] - (data_x[i, 0] * solution[0][k, 0] + solution[0][k, 1])) * solution[1][j, k] > 0)
            if temp:
                class_temp[j, 0] = 1
